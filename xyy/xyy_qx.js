 ...
 * 
 * 3. 环境变量支持 (可在 BoxJS 或持久化存储中配置):
 * key: xyy_cookie (格式: ysmuid=xxx)
 * key: xyy_unionid (格式: oZdBp0y-xxx)
 * key: xyy_entry_url (最新抓包入口/接口域名，可选)
 * key: xyy_max_read (单次运行最大阅读篇数，默认 30)
 * 
 * 多账号配置：支持使用 "&" 或 "@" 或 "换行" 分隔。
 * ==============================================================
 */

const $ = new Env("小阅阅全自动阅读");

// 默认配置参数
const DEFAULT_HOST_DOMAIN = "http://c4dfacd.ohqnjl.asia";
const DEFAULT_MAX_READ = 30;
const READ_DURATION_MIN = 8;
const READ_DURATION_MAX = 10;
...
(async () => {
    ...
    // 2. 否则为定时任务模式
    console.log("==================================================");
    console.log("   小阅阅全自动阅读脚本 v1.0.0 (Quantumult X 版)  ");
    console.log("==================================================");

    const accounts = getAccounts();
    if (accounts.length === 0) {
        $.notify("小阅阅", "运行失败 ❌", "未检测到有效的 xyy_cookie 或 xyy_unionid，请先通过微信重写抓包！");
        $done();
        return;
    }
...
async function processSingleRead(hostDomain, headers, unionid, readIdx, maxRead) {
    ...
    if (!rid) {
        ...
        return { success: false, gold: 0, msg: "无效RID" };
    }
    ...
    try {
        ...
        return { success: false, gold: 0, msg: `dudu网络异常: ${e}` };
    }
    ...
    try {
        ...
        return { success: false, gold: 0, msg: `结算网络异常: ${e}` };
    }
    ...
    if (resJinright.errcode === 0) {
        ...
        if (earnedGold > 0) {
            ...
            return { success: true, gold: earnedGold, msg: "成功" };
        } else {
            console.log(`  └─ ⚠️ [提示] 结算成功但增加金币为 0，响应: ${JSON.stringify(data)}`);
            return { success: true, gold: 0, msg: "金币为0" };
        }
    } else {
...
async function runAccount(account, defaultEntryUrl, maxRead) {
    ...
    if (statusBefore && statusBefore.errcode === 0) {
        ...
        console.log(`📊 初始状态: 今日已读 ${bData.day_read || 0} 篇, 今日金币: ${bData.day_gold || 0}, 账户余额: ${bData.last_gold || 0}`);
    } else {
    ...
    while (successReads < maxRead) {
        ...
        if (successReads < maxRead) {
            const intervalSec = (Math.random() * 2 + 3).toFixed(1);
    ...
    if (statusAfter && statusAfter.errcode === 0) {
        ...
        goldMsg = `今日已读 ${aData.day_read || 0} 篇, 今日金币: ${aData.day_gold || 0}, 账户余额: ${aData.last_gold || 0}`;
        console.log(`\n🎉 [账号${idx}] 运行总结:`);
    ...
    return `账号 [${idx}] 完成阅读 ${successReads} 篇，新增 +${totalEarnedGold} 金币。最新状态: ${goldMsg}`;
}
