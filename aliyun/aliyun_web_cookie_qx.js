/*
阿里云社区 Cookie 抓取模块 - 多平台版 (Surge/Quantumult X)
@Author: z.W.
@Date: 2026-05-21
@Version: 1.0.2
@Description: 
  ...
  不执行任何任务脚本
  支持Surge模块参数配置，支持Quantumult X (通过BoxJs或本地缓存配置)
  支持用户名去重，避免重复创建青龙变量

获取 Cookie 方式: 阿里云 APP - 首页 - 积分商城 或 网页版登录后进入个人中心

参数说明:
  [Surge] 通过模块argument配置
  [Quantumult X] 请通过BoxJs配置或手动执行脚本设置本地缓存
...
更新日志:
  v1.0.2 - 适配 Quantumult X，提供多平台兼容层
        ...
        return new Promise((resolve) => {
            if (typeof $httpClient !== 'undefined') {
                ...
                $task.fetch(options).then(resp => {
                    resolve({ err: null, resp: resp, body: resp.body });
                }, err => {
                    resolve({ err: err.error || err, resp: null, body: null });
                });
            }
        });
    },
...
// 青龙配置 - 优先从argument参数获取(Surge)，然后从本地存储获取(QX/BoxJs)
...
async function updateQlEnv(token, envId, value) {
    ...
    const body = JSON.stringify({
        ...
        name: qlDataName,
        value: value,
        remarks: '阿里云社区Cookie - Surge/QX自动同步 ' + new Date().toLocaleString()
    });
