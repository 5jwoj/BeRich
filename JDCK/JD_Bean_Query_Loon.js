 ...
 *
 * @配置方式
 * 通过 BoxJS 订阅「BeRich Loon 合集」，在「京豆资产查询 (Loon)」App 中填写：
 *   - jd_ql_url             青龙面板地址
...
(async () => {
    ...
    const scriptName = MANUAL_CONFIG.script_name || getBoxJSSetting("jd_asset_script_name") || "jd_task_assets";
    const userPinStr = MANUAL_CONFIG.pin || getBoxJSSetting("jd_local_pin") || "";
    ...
    if (!ql_url || !ql_client_id || !ql_client_secret) {
        $notification.post(
            "⚠️ 【京豆查询】配置未填写",
            "BoxJS 青龙信息未设置",
            "请在 BoxJS 订阅「BeRich Loon 合集」→「京豆资产查询」中填写青龙面板地址、Client ID 和 Client Secret。"
        );
        $done({});
        return;
    }
