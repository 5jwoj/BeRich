/*
 * 📦 JD Cookie 过期检测 - 青龙面板专属版 (Surge)
 * Version: v1.0.0
 * Author: z.W.
 *
 * 功能说明:
 *   1. 从 BoxJS 读取青龙面板信息 (地址/Client ID/Secret)
 *   2. 读取 BoxJS 中指定的京东账号 Pin (jd_check_pin)，精准检测指定账号
 *   3. 智能扫描青龙最近一次运行的日志，判断账号是否出现 "Cookie失效"、"账号已过期"、"未登录" 等提示
 *   4. 一旦检测到目标账号过期或失效，第一时间通过本地通知推送预警
 *
 * @配置方式
 * 通过 BoxJS 订阅「BeRich Surge 合集」，在「JD账号过期检测 (Surge)」App 中填写：
 *   - jd_ql_url                青龙面板地址
 *   - jd_ql_client_id          青龙 Client ID
 *   - jd_ql_client_secret      青龙 Client Secret
 *   - jd_cookie_check_script   检测目标脚本名称（默认 jd_task_assets）
 *   - jd_check_pin             指定检测的 Pin（留空检测全部）
 *
 * BoxJS 订阅地址：https://raw.githubusercontent.com/5jwoj/BeRich/main/boxjs/BeRich_Surge.boxjs.json
 *
 * Surge 定时任务配置示例（每天 6:35 ~ 23:35 每小时执行一次）：
 * 35 6-23 * * * script-path=https://raw.githubusercontent.com/5jwoj/BeRich/main/JDCK/JD_Cookie_Check_Surge.js,script-update-interval=86400,timeout=60,tag=JD账号过期检测
 */
...
(async () => {
    ...
    const scriptName = getBoxJSSetting("jd_cookie_check_script") || getBoxJSSetting("jd_asset_script_name") || "jd_task_assets";
    const userPinStr = getBoxJSSetting("jd_check_pin") || getBoxJSSetting("jd_local_pin") || "";
    ...
    if (!ql_url || !ql_client_id || !ql_client_secret) {
        $notification.post(
            "⚠️ 【账号过期检测】配置未填写",
            "BoxJS 青龙信息未设置",
            "请在 BoxJS 订阅「BeRich Surge 合集」→「JD账号过期检测」中填写青龙面板地址、Client ID 和 Client Secret。"
        );
        $done({});
        return;
    }
