...
(async () => {
        console.log("🚀 脚本实例已创建 (v1.0.3)");

        if (typeof $request !== 'undefined') {
                getCookie();
        } else {
                // 只有在手动点击“运行”或 Cron 定时触发时才会进入这里 (且没有 $request)
                // 但注意：Surge 的“测试脚本”功能可能会提供假的 $request (apple.com)，
                // 所以上面 getCookie 里还需要校验 URL。
...
function getCookie() {
        ...
        // 关键修复：过滤非阿里云的请求（Surge 手动测试生成的假请求）
        if ($request.url.indexOf("aliyun.com") === -1) {
                console.log(`⚠️ 忽略非阿里云请求 URL: ${$request.url}`);
                console.log("💡 提示: 请不要在 Surge 点击'运行脚本'来抓取 Cookie，而是去打开阿里云 App！");
                return;
        }
        ...
        if (!$request.headers) {
                console.log("❌ Critical: $request.headers 不存在");
                return;
        }

        const cookie = $request.headers['Cookie'] || $request.headers['cookie'];
        if (cookie) {
                console.log(`✅ 成功提取到 Cookie (长度: ${cookie.length})`);
                ...
                if (oldCookie !== cookie) {
                        ...
                        if (setRes) {
                                ...
                                console.log(`💾 Cookie 已保存 (前30位): ${cookie.substring(0, 30)}...`);
                        } else {
...
// Env 助手类 (简化版，兼容 Surge/Loon/QuanX/NodeJS)
