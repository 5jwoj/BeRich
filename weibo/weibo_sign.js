...
console.log('--- Weibo Script Loaded (v1.0.6) ---')

const TOKEN_KEY = 'sy_token_wb'
const COOKIE_KEY = 'wb_cookie'

// 更加严谨的模式判断：
// 1. 如果 $request 存在且有 URL，通常是拦截模式
// 2. 如果是手动运行或 Cron，通常 $request 为空或未定义
const isRequest = typeof $request !== 'undefined' && typeof $request.url !== 'undefined'

let wbsign = ''
let paybag = ''

if (isRequest) {
  console.log('Mode: HTTP Request (Interception)')
  ...
  console.log('Mode: Manual/Cron (Task Execution)')
  main().catch(e => {
...
async function main() {
  ...
  console.log('Script Version: v1.0.6')

  let tokens = $persistentStore.read(TOKEN_KEY)
  let cookies = $persistentStore.read(COOKIE_KEY)

  console.log(`Data Check - Token: ${tokens ? 'Exists' : 'Empty'}, Cookie: ${cookies ? 'Exists' : 'Empty'}`)

  if (!tokens) {
    console.log('No Weibo Token found in PersistentStore. Please open Weibo app to capture it.')
  ...
  console.log(`Starting processing for ${tokenArr.length} account(s)...`)
  ...
  for (let i = 0; i < tokenArr.length; i++) {
    ...
    if (!cookie) {
      ...
      console.log(`[Account ${i + 1}] Skip wallet sign-in: SUB cookie not found`)
    } else {
...
function getCookie() {
  ...
  if (url.includes('users/show') && url.includes('gsid=')) {
    console.log('Maching Token capturing rule...')
    ...
    if (!from || !uid || !gsid || !s) {
      console.log('Incomplete token parameters in URL.')
      return
    }
    ...
    if (old && old.includes(token)) {
      console.log('Token already exist, skip.')
      return
    }
  ...
  if (cookieHeader && cookieHeader.includes('SUB=')) {
    console.log('Maching Cookie capturing rule...')
    ...
    if (old && old.includes(cookie)) {
      console.log('Cookie already exist, skip.')
      return
    }
...
function weiboSign(token) {
  return new Promise(resolve => {
    $httpClient.post(
      ...
      (err, resp, data) => {
        if (err) {
          console.log(`Weibo Sign-in Request Error: ${err}`)
          wbsign = `每日签到：请求失败 (${err})`
        } else {
          console.log(`Weibo Sign-in Response: ${data}`)
          try {
            ...
            if (res.status === 10000) {
              ...
              wbsign = `每日签到：失败 (${res.msg || res.errmsg || '错误代码 ' + res.status})`
            }
          } catch (e) {
            wbsign = '每日签到：解析失败'
...
function paySign(token, cookie) {
  return new Promise(resolve => {
    $httpClient.post(
      ...
      (err, resp, data) => {
        if (err) {
          console.log(`Wallet Sign-in Request Error: ${err}`)
          paybag = `钱包签到：请求失败 (${err})`
        } else {
          console.log(`Wallet Sign-in Response: ${data}`)
          try {
            let res = JSON.parse(data)
            if (res.status === 1) {
              ...
              paybag = `钱包签到：失败 (${res.msg || '错误代码 ' + res.status})`
            }
          } catch (e) {
            paybag = '钱包签到：解析失败'
            console.log('JSON Parse Error: ' + e)
